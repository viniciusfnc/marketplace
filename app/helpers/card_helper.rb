...
module CardHelper
  def card_report(report)
    header_div = tag.div class: 'card-header', style: 'text-align: center;' do
      concat content_tag :h5, report[:name], class: 'card-title'
    end

    body_div = tag.div class: 'card-body', style: 'text-align: center;' do
    ...
    footer_div = tag.div class: 'card-footer', style: 'text-align: center;' do
